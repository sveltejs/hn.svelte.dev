...
var mime = new Mime_1$1(standard$1, other);
...
async function render_page({
	...
	const preload_context = {
		...
		fetch: async (url, opts) => {
			...
			const resolved = Url.resolve(path$1, parsed.pathname);
	...
	const App = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(path.resolve(`${files}/app.js`))); });

	try {
		const match = page.pattern.exec(path$1);
		...
		const parts = await Promise.all([{ component: manifest.layout, params: [] }, ...page.parts].map(async (part, i) => {
			...
			const mod = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(path.resolve(`${files}/components/${part.component.name}.js`))); });
		...
		const js_deps = new Set([...client.deps.__entry__.js]);
		const css_deps = new Set([...client.deps.__entry__.css]);
...
async function render_route({
	...
	const mod = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(path.resolve(`${files}/routes/${route.name}.js`))); });
	...
	if (handler) {
		const params = {};
		const match = route.pattern.exec(path$1);
		...
		return {
			status: 501,
			body: `${method} is not implemented for ${path$1}`
		};
	}
}
...
function parse_querystring(querystring) {
	const query = {};
	new Url.URLSearchParams(querystring).forEach((value, key) => {
...
async function render({
	...
	if (route) {
		return render_route({ method: 'GET' /* TODO */, files, host, path, query, route });
	}

	if (page) {
		return render_page({ static_dir, template, manifest, client, files, host, path, query, page });
	}
...
const template = fs__default['default'].readFileSync('build/app.html', 'utf-8');
